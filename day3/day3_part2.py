...
#fabric = np.zeros((1000,1000),dtype=list())
w, h = 1000, 1000;
...
# loop over check if only 1 is there
...
while z < len(claims):
	...
	while y + int(claims[z][2]) < (int(claims[z][2]) + int(claims[z][4])):
		x = 0
		while x + int(claims[z][1]) < (int(claims[z][1]) + int(claims[z][3])):
			if fabric[x + int(claims[z][1])][y + int(claims[z][2])] == 1:
				unique += 1
				if unique == (int(claims[z][3]) * int(claims[z][4])):
					unique_ID = claims[z][0]
			x += 1
		y += 1
	z += 1
print unique_ID
